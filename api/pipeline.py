...
class Pipeline:
    ...
    # public methods
    ############################################################
    def create(self, token: t.Token, command: str, cloud: bool, **kwargs) -> t.PipelineId:
        headers = api_utils.get_auth_headers(token)
        in_data = {'command': command, 'cloud': cloud, **kwargs}
        if "tags" in kwargs:
            in_data["tags"] = self.sanitize_tags(in_data["tags"])
        response = request_utils.post(self.url + '/program/program', headers=headers, data=in_data)
    ...
    def get(self, token: t.Token, pipeline_id: t.PipelineId) -> dict:
        ...
        response = request_utils.get(self.url + f'/program/program/{pipeline_id}', headers=headers, params=data)
        return api_utils.get_data(response)

    def list(self, token: t.Token) -> list:
        headers = api_utils.get_auth_headers(token)
        response = request_utils.get(self.url + '/program/program/list', headers=headers)
        return api_utils.get_data(response)
    ...
    def update(self, token: t.Token, pipeline_id: t.PipelineId, params: dict, *args):
        ...
        response = request_utils.put(self.url + f'/program/program/{pipeline_id}', headers=headers, data=data)
        api_utils.get_data(response)

    def save_serialization(self, token: t.Token, pipeline_id: t.PipelineId,
                           serialization: str):
        headers = api_utils.get_auth_headers(token)
        data = {'serialization': serialization}
        response = request_utils.put(self.url + f'/program/program/{pipeline_id}/serialization', headers=headers, data=data)
        api_utils.get_data(response)

    def touch(self, token: t.Token, pipeline_id: t.PipelineId):
        headers = api_utils.get_auth_headers(token)
        response = request_utils.put(self.url + f'/program/program/{pipeline_id}/touch', headers=headers)
        api_utils.get_data(response)
    ...
    def get_history(self, token: t.Token, params: dict):
        headers = api_utils.get_auth_headers(token)
        response = request_utils.get(self.url + '/program/program/history', headers=headers, params=params)
        return api_utils.get_data(response)
