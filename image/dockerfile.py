    ...
    if _is_debian(linux_flavor):
        if linux_version == '10':  # buster
            ...
            raise UnsupportedPythonException(
                f'Unsupported Python version {pyvers}'
            )
        if pyvers is None:
        ...
        raise UnsupportedLinuxException(
            f'Unsupported Linux version {linux_flavor}.')
...
def get_python_version(user_image):
    for binary in ['python', 'python3', 'python3.5', 'python3.6', 'python3.7', 'python3.8']:
        try:
            version = _get_python_version(user_image, binary)
        except (subprocess.CalledProcessError, LowPException, packaging.version.InvalidVersion):
            pass
        else:
...
def _get_linux_flavor_and_version(user_image):
    output =  subprocess.check_output(
        ...
        shell=True, stderr=subprocess.PIPE).decode('utf8').strip()
