...
def build(node, build_mode=constants.BuildMode.DEPLOY_TO_CLOUD, tags=None,
          title=None, use_shell=True, retention=7):
    ...
    pipeline_id = api.Pipeline().create(
        token, command, cloud=cloud, retention=retention, tags=tags or [],
        title=title)
    ...
    def local_deploy():
        ...
        serialization_path = os.path.join(local_progdir, constants.ConductoPaths.SERIALIZATION)
...
def run_in_local_container(token, pipeline_id):
    ...
    serialization = f'{remote_basedir}/logs/{pipeline_id}/{constants.ConductoPaths.SERIALIZATION}'
    ...
    flags = [
        ...
        "-v", f"{local_basedir}:{remote_basedir}",
        ...
        "-e", f"CONDUCTO_BASE_DIR_VERIFY={remote_basedir}",
        "-e", f"CONDUCTO_LOCAL_BASE_DIR={local_basedir}",
        "-e", f"CONDUCTO_LOCAL_HOSTNAME={socket.gethostname()}",
    ]
    ...
    if conducto_in_docker:
        ...
        skinny_flags += [
            "-e", f"CONDUCTO_RUNNING_IN_DOCKER={conducto_in_docker}"]

        volname = "dotconducto_{}".format(uuid.uuid1().hex)

        docker_parts = ["docker", "create", "-v", remote_basedir, "--name", volname, "alpine:3.11", "/bin/true"]
        log.debug(" ".join(pipes.quote(s) for s in docker_parts))
        client_utils.subprocess_run(docker_parts, capture_output=capture_output, msg="error build dummy volume container")
        docker_parts = ["docker", "cp"] + [local_basedir + '/.', f"{volname}:{remote_basedir}"]
        log.debug(" ".join(pipes.quote(s) for s in docker_parts))
        client_utils.subprocess_run(docker_parts, capture_output=capture_output, msg="error copying .conducto pipeline")
        docker_parts = ["docker", "run"] + skinny_flags + ["--volumes-from", volname] + [manager_image] + cmd_parts
        log.debug(" ".join(pipes.quote(s) for s in docker_parts))
        client_utils.subprocess_run(docker_parts, capture_output=capture_output, msg="Error starting manager container")
    else:
        # Pull manager image if from dockerhub.
        if manager_image.startswith('conducto/'):
            docker_parts = ["docker", "pull", manager_image]
            log.debug(" ".join(pipes.quote(s) for s in docker_parts))
            client_utils.subprocess_run(docker_parts, capture_output=capture_output, msg="Error pulling manager container")
        # Run manager container.
        docker_parts = ["docker", "run"] + flags + [manager_image] + cmd_parts
        log.debug(" ".join(pipes.quote(s) for s in docker_parts))
        client_utils.subprocess_run(docker_parts, msg="Error starting manager container", capture_output=capture_output)
    ...
    if not _manager_debug():
        ...
        for _ in range(int(
            ...
            raise RuntimeError(f'no manager connection to pgw for {pipeline_id} after {constants.ManagerAppParams.WAIT_TIME_SECS} seconds')
...
def clean_log_dirs(token):
    ...
    local_basedir = os.path.join(
            constants.ConductoPaths.get_local_base_dir(), 'logs')
    if os.path.isdir(local_basedir):
        for subdir in os.listdir(local_basedir):
            if subdir not in pipeline_ids:
                shutil.rmtree(
                        os.path.join(local_basedir, subdir), ignore_errors=True)
